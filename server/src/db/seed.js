...
const admin = db
  ...
  .get("admin@ecoride.local");
if (!admin) {
  db.prepare(
    `INSERT INTO utilisateurs (nom, prenom, email, password_hash, role_id)
              VALUES (?,?,?,?,?)`,
  ).run(
    "Admin",
    "Eco",
...
if (!emp) {
  db.prepare(
    ...
    bcrypt.hashSync("employe123", 10),
    getRoleId("employe"),
  );
  console.log("Employe seeded: employe@ecoride.local / employe123");
}

// Seed one utilisateur (usager)
const user = db
  .prepare("SELECT * FROM utilisateurs WHERE email = ?")
  .get("user@ecoride.local");
if (!user) {
  db.prepare(
    `INSERT INTO utilisateurs (nom, prenom, email, password_hash, role_id)
              VALUES (?,?,?,?,?)`,
    ...
    getRoleId("utilisateur"),
  );
  console.log("Utilisateur seeded: user@ecoride.local / user123");
}
...
if (covCount === 0) {
  const insertCov =
    db.prepare(`INSERT INTO covoiturages (date_depart, heure_depart, lieu_depart, date_arrivee, heure_arrivee, lieu_arrivee, statut, nb_place, prix_personne)
                                VALUES (?,?,?,?,?,?,?,?,?)`);
