...
app.get('/api/rides', (req, res) => {
  ...
  if (from) results = results.filter(r => r.from.toLowerCase() === String(from).toLowerCase());
  if (to) results = results.filter(r => r.to.toLowerCase() === String(to).toLowerCase());
  ...
  if (maxDuration) results = results.filter(r => rideDurationMinutes(r) <= Number(maxDuration));
  if (minRating) results = results.filter(r => r.driver.rating >= Number(minRating));
  ...
  if ((!results || results.length === 0) && from && to) {
    const sameRoute = rides
      .filter(r => r.from.toLowerCase() === String(from).toLowerCase() && r.to.toLowerCase() === String(to).toLowerCase() && r.seats > 0)
  ...
  res.json({
    items: results.map(r => ({
      ...
      driver: { pseudo: r.driver.pseudo, rating: r.driver.rating, photo: r.driver.photo },
      seats: r.seats,
      price: r.price,
      eco: r.eco
    })),
