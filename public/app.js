...
function buildQuery(params) {
  ...
  return Object.keys(params)
    .filter(
      (k) => params[k] !== undefined && params[k] !== "" && params[k] !== null
    )
...
function rideItemTemplate(r) {
  return `<li class="card result">
    <div class="header">
      ...
      <div>
        <div><strong>${r.driver.pseudo}</strong> • ⭐ ${r.driver.rating.toFixed(
    1
  )}</div>
      ...
      ${
        r.eco
          ? '<span class="badge" title="Trajet écologique">Écologique</span>'
          : ""
      }
    </div>
    <div class="meta"><span>${r.departureTime} → ${r.arrivalTime} (${
    r.durationMin
  } min)</span><span>${r.seats} place(s)</span><span><strong>${
    r.price
  }€</strong></span></div>
...
async function onResults() {
  ...
  ["from", "to", "date"].forEach((k) => {
    const v = url.searchParams.get(k);
    if (v) qs(`[name=${k}]`).value = v;
  });
...
async function onDetail() {
  ...
  qs("#ride-detail").innerHTML = `
    ...
    <p><strong>Départ:</strong> ${
      r.departureTime
    } • <strong>Arrivée:</strong> ${r.arrivalTime} • <strong>Durée:</strong> ${
    (new Date("1970-01-01T" + r.arrivalTime + ":00Z") -
      new Date("1970-01-01T" + r.departureTime + ":00Z")) /
    60000
  } min</p>
    <p><strong>Conducteur:</strong> ${r.driver.pseudo} • ⭐ ${
    r.driver.rating
  }</p>
    <p><strong>Véhicule:</strong> ${r.vehicle.brand} ${r.vehicle.model} — ${
    r.vehicle.energy
  }</p>
    <p><strong>Prix:</strong> ${
      r.price
    }€ • <strong>Places restantes:</strong> ${r.seats} • ${
    r.eco ? '<span class="badge">Écologique</span>' : ""
  }</p>
    <p><strong>Préférences du conducteur:</strong> ${
      r.preferences.fumeur ? "Fumeur" : "Non-fumeur"
    } • ${r.preferences.animaux ? "Animaux OK" : "Sans animaux"}</p>
  `;
  qs("#reviews").innerHTML = (r.reviews || [])
    .map((v) => `<li>⭐ ${v.note} — ${v.comment}</li>`)
    .join("");
}
